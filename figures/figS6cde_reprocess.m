...
load('figS5b_controller_16-15.mat')
load('figS5b_openloop_16-15.mat')


%% RUN parallel simulations to find deterministic steady states
tic
parfor traj_cntr=1:num_trajs
    ...
    % make relative trajectories absolute
    ts{traj_cntr}=rel_ts{traj_cntr};
    psens_trajs{traj_cntr}=rel_psens_trajs{traj_cntr}.*steady_psens{traj_cntr};
    l_trajs{traj_cntr}=rel_l_trajs{traj_cntr}.*steady_l{traj_cntr};
    D_trajs{traj_cntr}=rel_D_trajs{traj_cntr}.*steady_D{traj_cntr};
    ...
    % record trajectories
    ts_openloop{traj_cntr}=rel_ts_openloop{traj_cntr};
    psens_trajs_openloop{traj_cntr}=rel_psens_trajs_openloop{traj_cntr}.*steady_psens_openloop{traj_cntr};
    l_trajs_openloop{traj_cntr}=rel_l_trajs_openloop{traj_cntr}.*steady_l_openloop{traj_cntr};
    D_trajs_openloop{traj_cntr}=rel_D_trajs_openloop{traj_cntr}.*steady_D_openloop{traj_cntr};
end
toc
...
for traj_cntr=1:num_trajs
    ...
    plot(ts_openloop{traj_cntr}(t_in_frame)-ts_openloop{traj_cntr}(find(t_in_frame,1))-5,psens_trajs_openloop{traj_cntr}(t_in_frame)./psens_refmean_openloop,'Color',[0.6350 0.0780 0.1840 0.05])
end
% average trajectory
avg_psens_traj_openloop=mean(psens_trajs_openloop_concatenated,2);
plot(ts_openloop{1}(t_in_frame)-ts_openloop{traj_cntr}(find(t_in_frame,1))-5,avg_psens_traj_openloop(t_in_frame)./psens_refmean_openloop,'Color',[0.6350 0.0780 0.1840 1]);

% closed loop plots
for traj_cntr=1:num_trajs
    t_in_frame=(ts{traj_cntr}>=2.5)&(ts{traj_cntr}<=12.5);
    plot(ts{traj_cntr}(t_in_frame)-ts{traj_cntr}(find(t_in_frame,1))-5,psens_trajs{traj_cntr}(t_in_frame)./psens_refmean,'Color',[0 0.4470 0.7410 0.05])
end
% average trajectory
avg_psens_traj=mean(psens_trajs_concatenated,2);
plot(ts{1}(t_in_frame)-ts{1}(find(t_in_frame,1))-5,avg_psens_traj(t_in_frame)./psens_refmean,'Color',[0 0.4470 0.7410 1]);

ylim([0.85 1.1])
xlim([0 ts{1}(find(t_in_frame,1,'last')+1)-ts{1}(find(t_in_frame,1))]-[5 5])
xticks(-15:2.5:15)
...
for traj_cntr=1:num_trajs
    t_in_frame=(ts_openloop{traj_cntr}>=2.5)&(ts_openloop{traj_cntr}<=12.5);
    plot(ts_openloop{traj_cntr}(t_in_frame)-ts_openloop{traj_cntr}(find(t_in_frame,1))-5,l_trajs_openloop{traj_cntr}(t_in_frame)./l_refmean_openloop,'Color',[0.6350 0.0780 0.1840 0.01])
end

% average trajectory
avg_l_traj_openloop=mean(l_trajs_openloop_concatenated,2);
plot(ts_openloop{1}(t_in_frame)-ts_openloop{traj_cntr}(find(t_in_frame,1))-5,avg_l_traj_openloop(t_in_frame)./l_refmean_openloop,'Color',[0.6350 0.0780 0.1840 1]);

% closed loop plots
for traj_cntr=1:num_trajs
    t_in_frame=(ts{traj_cntr}>=2.5)&(ts{traj_cntr}<=12.5);
    plot(ts{traj_cntr}(t_in_frame)-ts{traj_cntr}(find(t_in_frame,1))-5,l_trajs{traj_cntr}(t_in_frame)./l_refmean,'Color',[0 0.4470 0.7410 0.01])
end

% average trajectory
avg_l_traj=mean(l_trajs_concatenated,2);
plot(ts{1}(t_in_frame)-ts{1}(find(t_in_frame,1))-5,avg_l_traj(t_in_frame)./l_refmean,'Color',[0 0.4470 0.7410 1]);
...
for traj_cntr=1:num_trajs
    t_in_frame=(ts_openloop{traj_cntr}>=2.5)&(ts_openloop{traj_cntr}<=12.5);
    plot(ts_openloop{traj_cntr}(t_in_frame)-ts_openloop{traj_cntr}(find(t_in_frame,1))-5,D_trajs_openloop{traj_cntr}(t_in_frame)./D_refmean_openloop,'Color',[0.6350 0.0780 0.1840 0.01])
end

% average trajectory
avg_D_traj_openloop=mean(D_trajs_openloop_concatenated,2);
plot(ts_openloop{1}(t_in_frame)-ts_openloop{traj_cntr}(find(t_in_frame,1))-5,avg_D_traj_openloop(t_in_frame)./D_refmean_openloop,'Color',[0.6350 0.0780 0.1840 1]);

% closed loop plots
for traj_cntr=1:num_trajs
    t_in_frame=(ts{traj_cntr}>=2.5)&(ts{traj_cntr}<=12.5);
    plot(ts{traj_cntr}(t_in_frame)-ts{traj_cntr}(find(t_in_frame,1))-5,D_trajs{traj_cntr}(t_in_frame)./D_refmean,'Color',[0 0.4470 0.7410 0.01])
end

% average trajectory
avg_D_traj=mean(D_trajs_concatenated,2);
plot(ts{1}(t_in_frame)-ts{1}(find(t_in_frame,1))-5,avg_D_traj(t_in_frame)./D_refmean,'Color',[0 0.4470 0.7410 1]);
