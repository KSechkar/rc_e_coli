%% DREAM_postproc_ameded.m
% Postprocessing script for the DREAM fitting of model parameters. Written
% by Jasper A. Vrugt as part of the DREAM package, minor amendments made by
% Kirill Sechkar where indicated
%%
...
% parameter names
par_names = {
    '\alpha_r:\alpha_a',... % metabolic gene transcription rate
    '\nu_{max}',...  % max. tRNA aminoacylatio rate
    'K_\epsilon=K_\nu',... % MM constants for translation elongation and tRNA charging rates
    'k_{cm}' % chloramphenicol binding rate constant
    };
...
% Add labels
xlabel('Number of generations','fontsize',14,'fontweight','bold','fontname','arial');
ylabel('R_{stat}','fontsize',14,'fontweight','bold','fontname','arial');
% Add title
title('Convergence of sampled chains','fontsize',14,'fontweight','bold','fontname','arial');
...
while counter <= DREAMPar.d
    ...
    if j == 1,
        % Add title
        title('Histograms of marginal distributions of individual parameters','fontsize',14,'fontweight','bold','fontname','arial');
    end;
    % Add x-labels
    evalstr = par_names(counter); xlabel(evalstr,'fontsize',14,'fontweight','bold','fontname','arial');
    % Then add y-label (only if j == 1 or j = r;
    if j == 1 | ( min(abs(j - ([1:r]*t+1))) == 0 ),
        ylabel('Marginal density','fontsize',14,'fontweight','bold','fontname','arial');
    end;
...
%% EDITED BY K.S.!
num_points=1000; % number of points in the array of fitted density function values

cdfs=zeros(num_points,size(Pars,2)); % cumulative density function values
xs=zeros(num_points,size(Pars,2));% corresponding parameter values
...
while counter <= DREAMPar.d
    ...
    %%
    [N,X]=density(num_points,Pars(:,counter),[]);
    %% EDITED BY K.S.!
    % find mode of the distribution - this is the fitted parameter value
...
%     disp(['Posterior distribution mode for Par. ',num2str(counter),' is ',num2str(mode)])
    ...
    if j == 1,
        % Add title
        title('Marginal posterior density of individual parameters','fontsize',14,'fontweight','bold','fontname','arial');
    end;
    % Add x-labels
    evalstr = par_names(counter); xlabel(evalstr,'fontsize',14,'fontweight','bold','fontname','arial');
    % Then add y-label (only if j == 1 or j = r;
...
if ( DREAMPar.d <= 30 ),
    ...
    plotmatrix(Pars,'+r');
    % Add title
    title('Marginal distributions and two-dimensional correlation plots of posterior parameter samples','fontsize',14,'fontweight','bold','fontname','arial');
end;
% % % % % % % % % % % % % figure(fig_number); fig_number = fig_number + 1;
% % % % % % % % % % % % % [multi_N,multi_X]=multivar_density(Pars,100);
% % % % % % % % % % % % % [~,multi_mode_index]=max(multi_N); % find index of the mode
% % % % % % % % % % % % % multi_mode=X(multi_mode_index);
% % % % % % % % % % % % % disp(multi_mode)
...
while counter <= DREAMPar.d
    ...
    if j == 1,
        % Add title - EDITED BY K.S.!!
        title(['Autocorrelation plot of sampled parameters: ',par_names{counter}],'fontsize',14,'fontweight','bold','fontname','arial');
    end;
    
    % Add x-labels - EDITED BY K.S.!!
    xlabel(par_names(counter));
    title(['Autocorrelation plot of sampled parameters: ',par_names{counter}],'fontsize',14,'fontweight','bold','fontname','arial');
...
if ~isempty(FX_post),
    ...
    while counter <= size(FX_post,2),
        ...
        if j == 1,
            % Add title
            title('Marginal posterior density of individual summary statistics','fontsize',14,'fontweight','bold','fontname','arial');
        end;
        % Add x-labels
        evalstr = strcat('S_',num2str(counter)); xlabel(evalstr,'fontsize',14,'fontweight','bold','fontname','arial');
        % Then add y-label (only if j == 1 or j = r;
        if j == 1 | ( min(abs(j - ([1:r]*t+1))) == 0 ),
            ylabel('Marginal density','fontsize',14,'fontweight','bold','fontname','arial');
        end;
...
for j = 1:DREAMPar.d,
    ...
    eval(char(evalstr));
    % Add a title
    xlabel('Sample number of chain','fontsize',14,'fontweight','bold','fontname','arial');
    % Then add a y-label
    evalstr = par_names{j}; ylabel(evalstr,'fontsize',14,'fontweight','bold','fontname','bold','fontname','arial'); % EDITED BY K.S.!
    % Then add title
    title('Chain convergence plot','fontsize',14,'fontweight','bold','fontname','bold','fontname','arial');
end;
...
if ~isempty(fx_post),
    ...
    % Add labels
    xlabel('y','fontsize',14,'fontweight','bold','fontname','bold','fontname','arial');
    ylabel('x','fontsize',14,'fontweight','bold','fontname','bold','fontname','arial');
    title('95% Posterior simulation uncertainty ranges (homoscedastic error!)','fontsize',14,'fontweight','bold','fontname','bold','fontname','arial');
    % -------------------------------------------------------------------------
...
if exist('ModPred'),
    ...
    % Then add title
    title('Autocorrelation plot of residuals','fontsize',14,'fontweight','bold','fontname','bold','fontname','arial');
    
    % Define ylabel
    ylabel('Autocorrelation','fontsize',14,'fontweight','bold','fontname','arial');
    ...
    % Define xlabel
    xlabel('Standard normal quantiles','fontsize',14,'fontweight','bold','fontname','arial');
    
    % Define ylabel
    ylabel('Quantiles of posterior sample','fontsize',14,'fontweight','bold','fontname','arial');
    
    % Then add title
    title('QQ plot of posterior sample versus standard normal distribution','fontsize',14,'fontweight','bold','fontname','bold','fontname','arial');
